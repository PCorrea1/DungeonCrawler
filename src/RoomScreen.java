...
public class RoomScreen extends Application {
    ...
    public RoomScreen(Player myPlayer) {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        primaryStage.setTitle("Room Screen - Room " + myPlayer.getCurrentRoom().getId() % 10);
        ...
        /* Room Number Text */
        Text text = new Text("You are in Room " + myPlayer.getCurrentRoom().getId() % 10);
        ...
        /* instantiate enemy */
        enemy = new Enemy(myPlayer.getDifficulty(), myPlayer.getCurrentRoom().getId() % 10);
        ...
        /* Populate Dungeon Doors, Place in Panes */
        ...
        door3.setOnMouseClicked(event -> {
            doorCheck3A(primaryStage);
        });

        if (myPlayer.getCurrentRoom().equals(myPlayer.getPath().getMyPlayerPath().getLast())) {
        ...
        if (myPlayer.getCurrentRoom().isCleared()) {
            firstEnemy = new ImageView(new Image("./media/defeatedEnemy.png"));
            enButton.setGraphic(firstEnemy);
            enButton.setStyle("-fx-background-color: transparent;");
            doorCheck4();
        } else {
            firstEnemy = new ImageView(new Image(enemy.getAvatar()));
            enButton.setGraphic(firstEnemy);
            enButton.setStyle("-fx-background-color: transparent;");
            enButton.setOnMouseClicked(event -> {
                if (!(enemy.isDead())) {
                    int damageToEnemy = myPlayer.getMyWeapon().getImpactDelta();
                    enemy.takeDamage(damageToEnemy);
                    myPlayer.setDamageDealt(myPlayer.getDamageDealt() + damageToEnemy);
                    ...
                    System.out.println("Damage Currently Dealt: " + myPlayer.getDamageDealt());
                    if (myPlayer.getHealth() <= 0) {
                        defeatScreenCue(primaryStage);
                    }
                    if (enemy.getHealth() <= 0) {
                        enemy.setDead(true);
                        myPlayer.getCurrentRoom().setCleared(true);
                    }
                } else {
                    if (myPlayer.getHealth() <= 0) {
                        defeatScreenCue(primaryStage);
                    ...
                    if (myPlayer.getCurrentRoom().isCleared()) {
                        ...
                        myPlayer.setMonstersKilled(myPlayer.getMonstersKilled() + 1);
                        System.out.println("Monsters Killed: " + myPlayer.getMonstersKilled());
                    }
        ...
        hBox4.getChildren().addAll(playerAvatar, enButton);
        ...
        primaryStage.show();
        askAboutChallenge(primaryStage);
    }
    ...
    private void askAboutChallenge(Stage primaryStage) {
        ...
        if (result.get() == ButtonType.OK) {
            ...
            ChallengeRoomScreen challengeRoom = new ChallengeRoomScreen(myPlayer);
            myPlayer.setPrevPrevRoom(myPlayer.getPrevRoom());
            myPlayer.setPrevRoom(myPlayer.getCurrentRoom());
